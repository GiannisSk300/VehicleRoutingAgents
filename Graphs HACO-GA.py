...
class HACO_GA:
    def __init__(self, distances, coordinates, num_ants=100, beta=3, gamma=2, population_size=20, generations=20000,
    ...
    def solve_tsp(self):
        ...
        self.validate_all_customers_visited(self.best_solution)
        return self.best_solution, self.best_cost

    def validate_all_customers_visited(self, solution):

        if isinstance(solution, int):
            print("Error: Solution is an integer, expected a list of routes.")
            return False

        if not isinstance(solution, list):
            print(f"Error: Unexpected solution type: {type(solution)}")
            return False
        ...
        if isinstance(solution[0], list):
            ...
            visited = set(customer for route in solution for customer in route if customer != 0)
        else:
            # Solution is a single route
            visited = set(customer for customer in solution if customer != 0)

        all_customers = set(range(1, self.num_customers + 1))

        if visited == all_customers:
            print("All customers are visited.")
            return True
        else:
            missing = all_customers - visited
            print(f"Not all customers are visited. Missing customers: {missing}")
            return False
...
class MFCMTSP:
    ...
    def solve_truck_drone(self):
        print("Solving TSP for truck and drone...")
        sol_T, M_T = self.solve_truck_tsp()
        sol_D = deque()
        M_D = 0

        while M_T > M_D:
            min_cost, sol_xy, x = float('inf'), [], None

            for i in range(1, len(sol_T)):
                if self.drone_accessible[sol_T[i] - 1]:
                    curr_cost = self.calculate_subset_cost([sol_T[i]], 2)
        ...
        return list(sol_T), list(sol_D), max(M_T, M_D)

    def solve_truck_motorbike(self):
        print("Solving TSP for truck and motorbike...")
        sol_T, M_T = self.solve_truck_tsp()
        sol_M = deque()
        M_M = 0

        while M_T > M_M:
            k_V = self.vehicles[1]['capacity']
            min_cost, sol_xy, x, y = float('inf'), [], None, None

            for i in range(1, len(sol_T) - k_V):
                subset = sol_T[i:i + k_V]
                if self.is_valid_subset(subset, 1):
                    curr_cost = self.calculate_subset_cost(subset, 1)
                    if curr_cost < min_cost:
                        min_cost = curr_cost
                        sol_xy = subset
                        x, y = i, i + k_V - 1
        ...
        return list(sol_T), list(sol_M), max(M_T, M_M)
    ...
    def solve(self):
        ...
        # Validate truck solution
        self.validate_solution(sol_T, "Initial truck solution")
        ...
        while M_T > M_M or M_T > M_D:
            ...
            if vehicle_type == 'motorbike' and min_cost + M_M < M_T:
                ...
                M_T += self.calculate_subset_cost([sol_T[x - 1], sol_T[y + 1]], 0)
                sol_T = sol_T[:x] + sol_T[y + 1:]
                # Validate after offloading to motorbike
                self.validate_solution(sol_T, "Truck solution after motorbike offload")
                self.validate_solution(sol_M, "Motorbike solution")
                ...
                M_T += self.calculate_subset_cost([sol_T[x - 1], sol_T[x + 1]], 0)
                sol_T = sol_T[:x] + sol_T[x + 1:]
                # Validate after offloading to drone
                self.validate_solution(sol_T, "Truck solution after drone offload")
                self.validate_solution(sol_D, "Drone solution")
            else:
        ...
        self.validate_solution(sol_T, "Final truck solution")
        self.validate_solution(sol_M, "Final motorbike solution")
        self.validate_solution(sol_D, "Final drone solution")
        self.validate_all_customers_visited([sol_T, sol_M, sol_D])

        return list(sol_T), list(sol_M), list(sol_D), max(M_T, M_M, M_D)

    def validate_solution(self, solution, description):
        visited = set(customer for customer in solution if customer != 0)

    def validate_all_customers_visited(self, solutions):
        all_visited = set()
        for solution in solutions:
            all_visited.update(customer for customer in solution if customer != 0)

        all_customers = set(range(1, self.num_customers + 1))

        if all_visited == all_customers:
            print("All customers are visited across all vehicles.")
            return True
        else:
            missing = all_customers - all_visited
            print(f"Not all customers are visited. Missing customers: {missing}")
            return False
...
def plot_tsp(mfcmtsp):
    plt.figure(figsize=(12, 10))
    plt.title('Figure 1: Single truck (TSP)', fontsize=16)
    plot_customers_and_depot(mfcmtsp)

    sol_T, _ = mfcmtsp.solve_truck_tsp()
    plot_route(sol_T, mfcmtsp, color='b', label='Truck')
...
def plot_pdstsp(mfcmtsp):
    plt.figure(figsize=(12, 10))
    plt.title('Figure 2: Truck and drone (PDSTSP)', fontsize=16)
    plot_customers_and_depot(mfcmtsp)

    sol_T, sol_D, _ = mfcmtsp.solve_truck_drone()
    plot_route(sol_T, mfcmtsp, color='b', label='Truck')
    plot_route(sol_D, mfcmtsp, color='g', label='Drone')
...
def plot_mtsp(mfcmtsp):
    plt.figure(figsize=(12, 10))
    plt.title('Figure 3: Truck and motorbike (mTSP)', fontsize=16)
    plot_customers_and_depot(mfcmtsp)

    sol_T, sol_M, _ = mfcmtsp.solve_truck_motorbike()
    plot_route(sol_T, mfcmtsp, color='b', label='Truck')
    plot_route(sol_M, mfcmtsp, color='r', label='Motorbike')
...
def plot_mfcmtsp(mfcmtsp):
    plt.figure(figsize=(12, 10))
    plt.title('Figure 4: Truck, motorbike and drone (mfcmTSP)', fontsize=16)
    plot_customers_and_depot(mfcmtsp)

    sol_T, sol_M, sol_D, _ = mfcmtsp.solve()
    plot_route(sol_T, mfcmtsp, color='b', label='Truck')
    plot_route(sol_M, mfcmtsp, color='r', label='Motorbike')
    plot_route(sol_D, mfcmtsp, color='g', label='Drone')
...
def plot_customers_and_depot(mfcmtsp):
    plt.scatter([coord[0] for coord in mfcmtsp.customers],
                [coord[1] for coord in mfcmtsp.customers],
                c='gray', s=20, label='Customers')
    plt.plot(mfcmtsp.depot[0], mfcmtsp.depot[1], 'rs', markersize=15, label='Depot')
    plt.xlim(0, mfcmtsp.area_size)
    plt.ylim(0, mfcmtsp.area_size)
    plt.xlabel('X Coordinate')
    plt.ylabel('Y Coordinate')


def plot_route(route, mfcmtsp, color, label):
    for i in range(len(route) - 1):
        start = mfcmtsp.depot if route[i] == 0 else mfcmtsp.customers[route[i] - 1]
        end = mfcmtsp.depot if route[i + 1] == 0 else mfcmtsp.customers[route[i + 1] - 1]
        plt.plot([start[0], end[0]], [start[1], end[1]], c=color, linewidth=1, alpha=0.7)
    plt.plot([], [], c=color, label=label, linewidth=2)  # For legend


if __name__ == "__main__":
    # Create a single instance of MFCMTSP
    mfcmtsp = MFCMTSP(num_customers=50, truck_speed=1, motorbike_speed=2, drone_speed=4, motorbike_capacity=3)

    # Generate separate figures for each case
    plot_tsp(mfcmtsp)
    plot_pdstsp(mfcmtsp)
    plot_mtsp(mfcmtsp)
    plot_mfcmtsp(mfcmtsp)
