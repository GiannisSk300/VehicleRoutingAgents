...
def run_dataset(filename):
    ...
    num_vehicles = int(filename.split('-k')[-1].split('.')[0])
    ...
    for run in range(num_runs):
        ...
        is_valid = aco_sa.validate_solution(best_solution)
    ...
    rd1 = (best_overall_cost - vrp.optimal_value) / vrp.optimal_value * 100
    rd2 = (average_best_cost - best_overall_cost) / best_overall_cost * 100

    return {
        'dataset': os.path.basename(filename),
        'best_cost': best_overall_cost,
        'best_solution': best_overall_solution,
        'best_run_time': best_overall_run_time,
        'average_best_cost': average_best_cost,
        'all_best_costs': all_best_costs,
        'optimal_value': vrp.optimal_value,
...
def save_results_to_excel(results, filename='amr_ga_results.xlsx'):
    wb = Workbook()
    ws = wb.active
    ws.title = "Results"

    headers = ['Dataset', 'Number of Vehicles', 'Optimal Value', 'Best Cost', 'Average Best Cost',
               'Run Time (s)', 'RD1 (%)', 'RD2 (%)', 'Valid Solution']
    ws.append(headers)
    ...
    for result in results:
        row = [
            ...
            round(result['best_cost'], 2),
            round(result['average_best_cost'], 2),
            round(result['best_run_time'], 2),
            round(result['rd1'], 2),
            round(result['rd2'], 2),
...
def main():
    datasets = [
        "A-n48-k7.vrp", "B-n31-k5.vrp", "B-n44-k7.vrp", "B-n51-k7.vrp",
        "B-n57-k7.vrp", "P-n19-k2.vrp", "P-n16-k8.vrp",
        "P-n20-k2.vrp", "P-n21-k2.vrp", "P-n22-k8.vrp", "P-n22-k2.vrp", "P-n23-k8.vrp", "E-n76-k14.vrp"
    ]
    ...
    for dataset in datasets:
        ...
        result = run_dataset(os.path.join('test', dataset))
    ...
    # Print a summary of invalid solutions
    invalid_solutions = [r for r in results if not r['is_valid']]
    if invalid_solutions:
        print("\nInvalid solutions found:")
